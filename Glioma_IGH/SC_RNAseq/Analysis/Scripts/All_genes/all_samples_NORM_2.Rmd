...
## UMAP
```{r, warning=FALSE}
seurat_obj <- RunUMAP(seurat_obj, dims = 1:pca, seed.use = saved.seed, )
...
```{r}
seurat_obj <- RunTSNE(seurat_obj, dims=1:pca,seed.use = saved.seed)
...
```{r}
seurat_obj <- FindNeighbors(seurat_obj, dims = 1:pca)
```
...
```{r}
clustree(all_test, prefix = "RNA_snn_res.")
```
