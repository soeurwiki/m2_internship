...
```{r}
metabo_obj <- readRDS(file = './results/rds/all_samples_metabo.rds')
...
## UMAP
```{r, warning=FALSE}
metabo_obj <- RunUMAP(metabo_obj, reduction = "harmony", dims = 1:pc, seed.use = saved.seed,verbose = FALSE )
...
DimPlot(metabo_obj, reduction = "umap", group.by = "line", label.size = 4, label = TRUE)+ NoLegend()
...
### Split by line
```{r}
DimPlot(metabo_obj, reduction = "umap", group.by="line", split.by = "condition")
DimPlot(metabo_obj, reduction = "umap", group.by = "line", split.by = "condition", label.size = 4, label = TRUE)+ NoLegend()
DimPlot(metabo_obj, reduction = "umap", group.by="line", split.by = "condition")+ NoLegend()
```

## tSNE
tSNE with default Perplexity 30 :
```{r}
metabo_obj <- RunTSNE(metabo_obj, reduction = "harmony", dims = 1:pc, seed.use = saved.seed,verbose = FALSE )
...
### Split by line
```{r}
DimPlot(metabo_obj, reduction = "tsne", group.by="line", split.by = "condition")
DimPlot(metabo_obj, reduction = "tsne", group.by = "line", split.by = "condition", label.size = 4, label = TRUE)+ NoLegend()
DimPlot(metabo_obj, reduction = "tsne", group.by="line", split.by = "condition")+ NoLegend()
```
...
```{r}
metabo_obj <- FindNeighbors(metabo_obj, reduction = "harmony", dims = 1:pc)
```

## Save seurat object  
```{r}
saveRDS(metabo_obj, file = "./results/rds/all_samples_FindNeighbors_metabo.rds")
```

## Optimize resolution  
```{r, message=FALSE}
keep <- colnames(metabo_obj[[]])[! grepl("^RNA_snn_res",colnames(metabo_obj[[]]))]
