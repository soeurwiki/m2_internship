...
```{r}
seurat_obj <- readRDS(file = './results/rds/all_samples_Mca.rds')
...
```
```{r, echo=FALSE}
HGT_brain_signatures <- readRDS( file= './results/rds/all_samples_HGT_brain_signatures.rds')
```
...
lapply(
  ...
  function(x)DEenrichRPlot(seurat_obj, ident.1 = x, assay = "RNA", max.genes = nb_maxgenes, return.gene.list = FALSE, enrich.database =  enrichR_db)
)
```

# Save seurat object  
```{r}
saveRDS(seurat_obj, file = "./results/rds/all_samples_Clusters.rds")
```
