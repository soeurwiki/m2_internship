---
title: "METABO -- Defining clusters for one sample"
author: "ATIA Safiya"
date: "2023-07-26"
params:
  samp: "samp"
...
```{r}
metabo_obj <- readRDS(file = paste0('./Lucille/results/rds/samples/', samp, '_metabo.rds'))
...
```{r}
metabo_obj <- RunTSNE(metabo_obj, reduction = "pca", dims = 1:pc, seed.use = saved.seed, check_duplicates = FALSE, verbose = FALSE )
...
## Save seurat object  
```{r}
saveRDS(metabo_obj, file = paste0("./Lucille/results/rds/samples/", samp, "_FindNeighbors_metabo.rds"))
```

## Optimize resolution  
```{r, message=FALSE}
keep <- colnames(metabo_obj[[]])[! grepl("^SCT_snn_res",colnames(metabo_obj[[]]))]
...
```{r}
clustree(res.seurat, prefix = "SCT_snn_res.")
```
...
```{r}
clustree(metabo_obj, prefix = "SCT_snn_res.")
```
