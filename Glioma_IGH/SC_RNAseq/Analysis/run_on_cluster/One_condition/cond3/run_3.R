...
if(args[1] == "all_mark"){
    ...
    seurat_obj <- readRDS(file = paste0('./Lucille/results/rds/samples/', samp, '_Markers.rds'))
    seurat_obj <- RunMCA(seurat_obj)
    saveRDS(seurat_obj, file = paste0('./Lucille/results/rds/samples/', samp, '_Mca.rds'))

    pdf("./Lucille/results/PlotMC.pdf")
    ...
    saveRDS(HGT_brain_signatures, file = paste0('./Lucille/results/rds/samples/', samp, '_HGT_brain_signatures.rds'))

} else if(args[1] == "metabo_mark"){
    print("metabo mark")
    name <- paste0(samp, "_metabo_3.html")
    ...
    print("metabo annot")
    name <- paste0(samp, "_metabo_4.html")
    ...
    seurat_obj <- readRDS(file = paste0('./Lucille/results/rds/samples/', samp, '_Markers_metabo.rds'))
    seurat_obj <- RunMCA(seurat_obj)
    saveRDS(seurat_obj, file = paste0('./Lucille/results/rds/samples/', samp, '_Mca_metabo.rds'))

    pdf("./Lucille/results/PlotMC_metabo.pdf")
    DimPlotMC(seurat_obj, reduction = "mca", group.by = 'orig.ident', features = c("APOE"), as.text = T)
    dev.off()
    ...
    # Performing per-cell hypergeometric tests against the gene signature collection
    HGT_brain_signatures <- RunCellHGT(seurat_obj, pathways = brain_signatures, dims = 1:pc, n.features = nb_maxgenes)
    saveRDS(HGT_brain_signatures, file = paste0('./Lucille/results/rds/samples/', samp, '_HGT_brain_signatures_metabo.rds'))
}
