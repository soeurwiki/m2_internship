...
seurat_obj <- readRDS(file = paste0('./results/rds/, sample, '_Mca.rds'))
...
DimPlot(seurat_obj, reduction = "umap",  split.by = "condition")+ NoLegend()
```
```{r}
DimPlot(seurat_obj, reduction = "tsne",  group.by = "seurat_clusters")
DimPlot(seurat_obj, reduction = "tsne",  split.by = "condition") + NoLegend()
```
...
```
```{r, echo=FALSE}
HGT_brain_signatures <- readRDS( paste0('./results/rds/', sample, '_HGT_brain_signatures.rds'))
```
...
# Save seurat object  
```{r}
saveRDS(seurat_obj, file = paste0("./results/rds/", sample, "_Clusters.rds"))
```
