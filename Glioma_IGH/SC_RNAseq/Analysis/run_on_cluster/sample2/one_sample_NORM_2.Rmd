...
seurat_obj <- readRDS( file = paste0("./results/rds/samples", sample, "_FindNeighbors.rds") )
...
```
```{r, eval=FALSE}
top <- cluster.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_log2FC)
DoHeatmap(seurat_obj, features = top$gene) + NoLegend()
```

### save markers lists
```{r}
saveRDS(cluster.markers, file = paste0( "./results/rds/samples/", sample, "_cluster.markers.rds") )
```

# Save seurat object  
```{r}
saveRDS(seurat_obj, file = paste0("./results/rds/samples/", sample, "_Markers.rds"))
```
